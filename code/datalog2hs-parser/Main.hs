...
-------todo
    let outputtuples = vOutputTuples (head (tail div_input))
    let clausulas = map (filterClauses (head (tail (tail div_input)))) outputtuples
    printaPgm outputtuples clausulas
